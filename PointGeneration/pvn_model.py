...
class PointCloudModelBlock(nn.Module):
    ...
    def __init__(
        ...
        *, 
        ...
        self.point_cloud_model = PVNModel(in_channels=dim, out_channels=dim, embed_dim=dim, dropout=dropout, 
    ...
    def apply_point_cloud_model(self, x: Tensor, t = None) -> Tensor:
...
class PointCloudTransformerModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self, num_layers: int, in_channels: int = 3, out_channels: int = 3, embed_dim: int = 64, **kwargs):
        ...
        self.input_projection = nn.Linear(in_channels, embed_dim)  # 1033 64
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        # print("ori x shape", x.sahpe)

        x = self.input_projection(inputs)
        # print("after pro x shape", x.sahpe)

        x = self.blocks(x)
        # print("after blocks x shape", x.sahpe)
