...
class AllDataFeatureTwoEEG(Dataset):
    ...
    def __init__(self, data_path, sub_list, train=True, time_len=250, test_mean=True, aug_data=False, point_path=''):
        ...
        self.point_data = np.zeros((self.name_list.shape[0], self.name_list.shape[1], 2048, 6))
        self.color_label = np.zeros((self.name_list.shape[0], self.name_list.shape[1]))

        if point_path == '':
            # point_path = self.data_path + 'point_cloud_simple/'
            point_path = self.data_path + 'pc_2048/'

            for ii in range(0, self.name_list.shape[0]):
                for jj in range(0, self.name_list.shape[1]):
                    # file_path = point_path + self.name_list[ii][jj][3:] + '.npy'  # 生成完整的路径
                    # print(file_path)

                    self.point_data[ii, jj] = np.load(point_path + self.name_list[ii][jj][3:] + '.npy') #  原始点云数据

                    self.color_label[ii, jj] = name2color_label[self.name_list[ii][jj][3:]]   # 颜色标签 共6种（0-5）

            for ii in range(0, self.point_data.shape[0]):
                for jj in range(0, self.point_data.shape[1]):
                    self.point_data[ii, jj] = self.pc_norm(self.point_data[ii, jj])  # 归一化
        else:
            ...
            for ii in range(0, self.name_list.shape[0]):
                for jj in range(0, self.name_list.shape[1]):
                    true_points_path = self.data_path + 'point_cloud_simple/' + self.name_list[ii][jj][3:] + '.npy'
                    true_points = np.load(self.data_path + 'point_cloud_simple/' + self.name_list[ii][jj][3:] + '.npy')
                    # print("true_points_path", true_points_path)
                    ply_name = f'{point_path}{app_ss}-{name_app_dir[self.name_list[ii][jj][3:]]}{self.name_list[ii][jj][3:]}-0.ply'
        ...
        self.cls_num = 72
        if not self.train:
            if self.test_mean:  # True
                self.eeg_data = np.mean(self.eeg_data, axis=3, keepdims=True)
                # print(f"test eeg data shape is {self.eeg_data.shape}") # (1, 72, 2, 1, 64, 600)  test
                self.eeg_data2 = np.mean(self.eeg_data2, axis=3, keepdims=True)
                # print(f"test eeg data shape is {self.eeg_data2.shape}") # (1, 72, 2, 1, 64, 250)
        ...
        # train name:(72, 8), point: (72, 8, 8192, 6), eegdata: (1, 72, 8, 2, 64, 600), eegdata2:(1, 72, 8, 2, 64, 250)
        # test  name:(72, 2), point: (72, 2, 8192, 6), eegdata: (1, 72, 2, 1, 64, 600), eegdata2:(1, 72, 2, 1, 64, 250)
    ...
    def __len__(self, ):
        num = 1
        for ii in range(len(self.eeg_data.shape) - 2): # 不包括最后两个维度 即（64，250/600）
    ...
    def __getitem__(self, idx):
        ...
        return {'name': name, 'eeg_data': eeg_data_new, 'eeg_data2': eeg_data2_new, 'cls_label': label,
