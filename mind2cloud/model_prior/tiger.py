...
from Mind2Cloud.modules_prior import SharedMLP, PVConv, PointNetSAModule, PointNetAModule, PointNetFPModule, Attention, \
    Swish
from .transformer_branch import DiT
from Mind2Cloud.modules_prior.voxelization import Voxelization
...
class Tiger_Transformer(nn.Module):
    ...
    def __init__(self, num_classes, embed_dim, use_att, dropout=0.1,
        ...
        self.transformer = DiT(depth=8, tok_num=256, hidden_size=128, latent_size=512,
                               output_channel=2048)  # latent_size=258
        ...
        layers, _ = create_mlp_components(in_channels=256, out_channels=[128, dropout, num_classes],
                                          # was 0.5
                                          classifier=True, dim=2, width_multiplier=width_multiplier)
    ...
    def forward(self, inputs, t):
        ...
        for fp_idx, fp_blocks in enumerate(self.fp_layers):
            ...
            #     (coords_list[-1], coords, torch.cat([features, temb], dim=1),
            #      in_features_list[-1], temb))

            features_tf = self.transformer(in_features_list[-2], t, coords_list[-2])  # 倒着访问
            # features = self.transformer(features, t, coords)  # 倒着访问
            # print("in_features_list[-2]", in_features_list[-2].shape)  # ([48, 128, 256])
            # print("coords_list[-2]", coords_list[-2].shape)  # ([48, 3, 256])
            features_tf = features_tf.transpose(1, 2)
            # print("features_tf", features_tf.shape)  # ([48, 2048, 256])
            ...
            #
            #     features = t_mask[:, :, None] * self.conv_remap(features) + (1 - t_mask[:, :, None]) * self.tf_remap(
            #         features_tf)
            #
            #     features = self.feature_adjust(features)
            #     print("features", features.shape)

        # print("final features", self.classifier(features_tf).transpose(1, 2).shape)  # ([48, 64, 2048])

        return self.classifier(features_tf).transpose(1, 2)

        # for fp_idx, fp_blocks in enumerate(self.fp_layers):
        #     if fp_idx == 1:
        #         features_tf = self.transformer(in_features_list[-1 - fp_idx], t, coords_list[-1 - fp_idx])  # 倒着访问
        #
        #         print(in_features_list[-1 - fp_idx].shape)  # ([48, 128, 256])
        #         print(coords_list[-1 - fp_idx].shape)
        #
        #         print("features_tf", features_tf.shape)  # ([48, 256, 256])
        #
        #     features, coords, temb = fp_blocks((coords_list[-1 - fp_idx], coords, torch.cat([features, temb], dim=1),
        #                                         in_features_list[-1 - fp_idx], temb))
        #
        #     print("features", features.shape)  # ([48, 256, 64])
        #     print("coords", coords.shape)  # ([48, 3, 64])
